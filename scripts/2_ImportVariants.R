...
if (!all.equal(colnames(snvs.nr), colnames(snvs.nv)) |
    ...
    stop("Sample names differ between some of the input NR and NV files. Please check the column names.")
}

# Remove leading zeroes and "-Dog" suffix from sample names
colnames(snvs.nr) = colnames(snvs.nv) = colnames(indels.nr) = colnames(indels.nv) = 
    gsub("(^0)|(^00)|(-Dog)", "", colnames(snvs.nr))

cat("Imported", nrow(snvs.metadata), "SNVs and", nrow(indels.metadata), "indels.\n\n")
...
colnames(snvs.nr)[idx] = colnames(snvs.nv)[idx] = colnames(indels.nr)[idx] = colnames(indels.nv)[idx] = 
    relabel
...
cat("\nFound", sum(duplicated(snvs.metadata[, c("CHROM", "POS", "ALT")])), "duplicated SNVs and", 
    sum(duplicated(indels.metadata[, c("CHROM", "POS")])), "duplicated indels.\n\n")
...
cat("\n\nFound", sum(tumour.only.idx), "tumour-only (somatic) SNVs and", 
    sum(tumour.only.indels.idx), "tumour-only indels.\n")
...
save(snvs.metadata, snvs.nr, snvs.nv, snvs.vaf, indels.metadata, indels.nr, indels.nv, indels.vaf, 
     samples, tumours, hosts, tumour.only.idx, tumour.only.indels.idx, tumour.unique.idx,
     tumour.unique.any.idx, tumour.unique.indels.idx, excluded, MIN.READS,
     file=OUTPUT$RDATA)
